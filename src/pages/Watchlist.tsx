  ...
  if (!user) {
    return (
      <div className="flex min-h-[80vh] flex-col items-center justify-center px-4 text-center">
        ...
        <h2 className="font-display text-2xl font-bold">{t("watch.waiting")}</h2>
      ...
      >
        <header className="mb-8">
          ...
          <h1 className="font-display text-3xl font-bold md:text-4xl">
            {t("watch.titlePrefix")} <span className="gradient-text">{t("watch.title")}</span>
          </h1>
          <p className="mt-1 text-sm text-muted-foreground">
            {watchlist.length} {watchlist.length === 1 ? t("watch.count.one") : t("watch.count.many")}
          </p>
        </header>
